...
export const useStripeCheckoutIndividual = () => {
  ...
  const setLoading = (planKey: string, loading: boolean) => {
  ...
  const isLoading = (planKey: string) => {
    return loadingStates[planKey] || false;
  };
  ...
  const createCheckout = async (planType: 'professional' | 'premium', billingPeriod: 'monthly' | 'annual' = 'monthly') => {
    ...
    // Evitar múltiplas chamadas simultâneas para o mesmo plano
    if (isLoading(planKey)) {
      console.log('SECURITY: Checkout already in progress for this plan, ignoring duplicate request');
      return;
    }

    try {
      setLoading(planKey, true);
      ...
      setLoading(planKey, false);
    }
  };

  const openCustomerPortal = async () => {
    const portalKey = 'customer_portal';
    
    // Evitar múltiplas chamadas simultâneas
    if (isLoading(portalKey)) {
      console.log('SECURITY: Portal opening already in progress, ignoring duplicate request');
      return;
    }

    try {
      setLoading(portalKey, true);
