-- Enhanced security for WhatsApp message tables
-- This migration addresses potential security gaps in WhatsApp message access

-- First, let's ensure all WhatsApp message tables have comprehensive RLS policies
...
-- 2. Strengthen whatsapp_mensagens_temp table policies (already mostly correct, but ensure consistency)
...
-- 3. Add enhanced security trigger for WhatsApp messages to log access attempts
CREATE OR REPLACE FUNCTION public.whatsapp_security_audit()
RETURNS TRIGGER AS $$
BEGIN
  -- Log any access to WhatsApp messages for security monitoring
  IF TG_OP = 'SELECT' THEN
    -- Only log if trying to access messages that don't belong to the user
    IF OLD.user_id != auth.uid() THEN
      PERFORM public.log_whatsapp_security_event(
        auth.uid(),
        'UNAUTHORIZED_MESSAGE_ACCESS_ATTEMPT',
        NULL,
        'CRITICAL',
        inet_client_addr(),
        NULL,
...
CREATE TRIGGER whatsapp_messages_security_audit
  AFTER SELECT OR INSERT OR UPDATE OR DELETE ON public.whatsapp_messages
...
CREATE TRIGGER whatsapp_mensagens_temp_security_audit
  AFTER SELECT OR INSERT OR UPDATE OR DELETE ON public.whatsapp_mensagens_temp
  FOR EACH ROW EXECUTE FUNCTION public.whatsapp_security_audit();
...
-- Ensure user_id cannot be NULL (prevent orphaned messages)
...
ALTER TABLE public.whatsapp_messages
ADD CONSTRAINT whatsapp_messages_content_length_check 
CHECK (length(content) <= 10000);

ALTER TABLE public.whatsapp_mensagens_temp
ADD CONSTRAINT whatsapp_mensagens_temp_content_length_check 
