-- Bug encontrado em teste manual: os "pontos" exibidos em toda a tela de
-- Fidelidade (Hall da Fama, tabela de clientes, modal VIP) são só um
-- cálculo em cima de payments (amount * points_per_currency), NUNCA
-- gravado como transação real em loyalty_point_transactions. Resultado:
-- redeem_loyalty_reward (que corretamente confere o saldo real no ledger)
-- sempre recusava o resgate com "saldo de pontos insuficiente", mesmo pra
-- clientes que a tela mostrava com pontos de sobra — porque o saldo real
-- sempre era zero, não existia nenhum evento de "ganhou pontos" gravado
-- em lugar nenhum do sistema.
--
-- Corrige creditando pontos de verdade (kind='earn') sempre que um
-- pagamento é marcado como pago, e faz o backfill dos pagamentos pagos
-- que já existem, pra igualar o saldo real ao que já era exibido.
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' AND OLD.status = 'pago' THEN
    RETURN NEW; -- já estava pago, não é uma nova confirmação de pagamento
  END IF;

  -- Idempotência: nunca credita duas vezes o mesmo pagamento (ex: updates
  -- subsequentes que não mudam o status, ou reprocessamento).
  IF EXISTS (
...
-- Backfill: pagamentos já pagos antes desta migration existir, que nunca
-- geraram transação nenhuma no ledger.
INSERT INTO public.loyalty_point_transactions
