-- CORREÇÃO DE SEGURANÇA PARA DADOS PESSOAIS DOS CLIENTES
-- Migração focada apenas nos elementos essenciais

-- 1. Primeiro, remover função existente que está causando conflito
DROP FUNCTION IF EXISTS public.get_user_plan(uuid);

-- 2. Recriar a função get_user_plan com parâmetros corretos
...
-- 3. Remover triggers existentes da tabela clients para evitar conflitos
...
-- 6. Aplicar trigger de validação CPF existente
CREATE TRIGGER clients_validate_trigger
  BEFORE INSERT OR UPDATE ON public.clients
  FOR EACH ROW EXECUTE FUNCTION public.validate_client_cpf();

-- 7. Atualizar políticas RLS para serem mais restritivas e seguras
...
-- 8. Criar políticas RLS mais seguras
...
-- 9. Criar função para acesso seguro aos dados dos clientes com mascaramento
...
INSERT INTO public.audit_logs (
  ...
  'SECURITY_ENHANCEMENT', 'Correção implementada para proteção de dados pessoais dos clientes',
  jsonb_build_object(
    'security_features', ARRAY[
      'Enhanced RLS policies with stricter validation',
      'Audit logging for all client data access',
      'Sensitive data masking function',
      'Input validation and sanitization',
      'CPF validation trigger'
    ],
    'fixes_applied', ARRAY[
      'Removed conflicting function definitions',
      'Added comprehensive audit trail',
      'Implemented data access controls',
      'Added secure data retrieval function'
    ],
    'timestamp', now()
  )
);