...
--
-- NOTA: esta migration é equivalente (menos comentários) à migration anterior
-- 20260725002735_d9319149-...sql, já aplicada em produção primeiro. Reescrita
-- de forma idempotente (IF NOT EXISTS / DO blocks) pra não quebrar se o
-- histórico completo de migrations for reaplicado do zero.
...
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
CREATE INDEX IF NOT EXISTS idx_client_packages_client_service_status ON public.client_packages(client_id, service_id, status, expires_at);
...
CREATE TABLE IF NOT EXISTS public.package_session_usages (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL,
...
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
CREATE INDEX IF NOT EXISTS idx_package_session_usages_client_package ON public.package_session_usages(client_package_id);
DROP TRIGGER IF EXISTS update_package_session_usages_updated_at ON public.package_session_usages;
...
END $$;
CREATE INDEX IF NOT EXISTS idx_payments_client_package ON public.payments(client_package_id) WHERE client_package_id IS NOT NULL;
...
DROP TRIGGER IF EXISTS trg_debit_package_session ON public.appointments;
