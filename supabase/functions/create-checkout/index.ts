...
const getCorsHeaders = (origin?: string) => {
  ...
  // Allow any lovableproject.com domain
  const isLovableDomain = origin && origin.includes('lovableproject.com');
  const isAllowed = origin && (allowedOrigins.includes(origin) || isLovableDomain);
  
  return {
    "Access-Control-Allow-Origin": isAllowed ? origin : allowedOrigins[2], // Default to main domain
